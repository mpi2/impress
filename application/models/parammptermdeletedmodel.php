...
/**
 * Parameter MP Term Deleted model
 */
class ParamMPTermDeletedModel extends CI_Model implements IUserIdCheckable, IRecyclable
{
    const TABLE = 'param_mpterm_deleted';
    ...
    private function _filterFields($arr)
    {
        ...
        $keys = delete_array_values($this->parammptermmodel->getFields(), 'deleted');
        return keep_array_keys($arr, $keys);
    }

    public function getByParamMPTermId($pid)
    {
        return $this->db->from(self::TABLE)
                        ->where('param_mpterm_id', $pid)
                        ->limit(1)
                        ->get()
                        ->row_array();
    }
    ...
    /**
    * @alias
    * @see ParamMPTermDeletedModel::purgeRecord()
    */
    ...
    /**
    * @param int|array $record either the id of a record in Param_MPTerm table
    ...
    */
    public function insert($record)
    {
        if (is_numeric($record)) {
            $this->load->model('parammptermmodel');
            $record = $this->parammptermmodel->getById($record);
        }
    ...
    */
    public function restore($deletedRecordId, array $origin)
    {
        ...
        $this->load->model('parammptermmodel');
        // $rec = $this->parammptermmodel->getById($deletedRecord['param_mpterm_id']);
        // if( ! empty($rec))
            // $deletedRecord['param_mpterm_id'] = null;
        $iid = $this->parammptermmodel->insert($deletedRecord, ChangeLogger::ACTION_UNDELETE);
    ...
    */
    public static function getRecyclableFields()
    {
        return array(
            ...
            static::R_FIELDS => array('mp_term', 'mp_id')
        );
    }
}
