...
/**
 * Parameter Option Deleted model
 */
class ParamOptionDeletedModel extends CI_Model implements IUserIdCheckable, IRecyclable
{
    const TABLE = 'param_option_deleted';
    ...
    private function _filterFields($arr)
    {
        ...
        $keys = delete_array_values($this->paramoptionmodel->getFields(), 'deleted');
        return keep_array_keys($arr, $keys);
    }

    public function getByParamOptionId($pid)
    {
        return $this->db->from(self::TABLE)
                        ->where('param_option_id', $pid)
                        ->limit(1)
                        ->get()
                        ->row_array();
    }
    ...
    /**
    * @alias
    * @see ParamOptionDeletedModel::purgeRecord()
    */
    ...
    /**
    * @param int|array $record either the id of a record in Param_Option table
    ...
    */
    public function insert($record)
    {
        if (is_numeric($record)) {
            $this->load->model('paramoptionmodel');
            $record = $this->paramoptionmodel->getById($record);
        }
    ...
    */
    public function restore($deletedRecordId, array $origin)
    {
        ...
        $this->load->model('paramoptionmodel');
        // $rec = $this->paramoptionmodel->getById($deletedRecord['param_option_id']);
        // if( ! empty($rec))
            // $deletedRecord['param_option_id'] = null;
        $iid = $this->paramoptionmodel->insert($deletedRecord, ChangeLogger::ACTION_UNDELETE);
