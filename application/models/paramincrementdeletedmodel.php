...
/**
* Parameter Increment Deleted model
*/

class ParamIncrementDeletedModel extends CI_Model implements IUserIdCheckable, IRecyclable
{
    const TABLE = 'param_increment_deleted';
    ...
    private function _filterFields($arr)
    {
        ...
        $keys = delete_array_values($this->paramincrementmodel->getFields(), 'deleted');
        return keep_array_keys($arr, $keys);
    }

    public function getByParamIncrementId($pid)
    {
        return $this->db->from(self::TABLE)
                        ->where('param_increment_id', $pid)
                        ->limit(1)
                        ->get()
                        ->row_array();
    }
    ...
    /**
    * @alias
    * @see ParamIncrementDeletedModel::purgeRecord()
    */
    ...
    /**
    * @param int|array $record either the id of a record in Param_Increment table
    ...
    */
    public function insert($record)
    {
        if (is_numeric($record)) {
            $this->load->model('paramincrementmodel');
            $record = $this->paramincrementmodel->getById($record);
        }
    ...
    */
    public function restore($deletedRecordId, array $origin)
    {
        ...
        $this->load->model('paramincrementmodel');
        // $rec = $this->paramincrementmodel->getById($deletedRecord['param_increment_id']);
        // if( ! empty($rec))
            // $deletedRecord['param_increment_id'] = null;
        $iid = $this->paramincrementmodel->insert($deletedRecord, ChangeLogger::ACTION_UNDELETE);
    ...
    */
    public static function getRecyclableFields()
    {
        return array(
            ...
            static::R_FIELDS => array('increment_string', 'increment_unit')
        );
    }
}
